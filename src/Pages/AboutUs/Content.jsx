import React from 'react'
import './Content.css'
export default function () {
  return (
    <div className='AboutUs'>
      <h1>About Us</h1>
      <div className='AboutUs-Content'>
        <p>Welcome to SolePalette!</p>
        <p>We are enthusiastic artists with a hobby that got extremely out of hand. See the result here, the website!</p>
        <p>Nice that you want to delve into our team and what we do. All shoes at csshoes.vn are made by hand, completely according to your wishes. We strive to provide you with the sneaker of your dreams.
          We greatly appreciate that you take the time to read about us.</p>
        <p>CsShoes stands for quality. We make all our products with the best quality leather paint from Angelus paints and we ensure that the design cannot be worn off.</p>
        <p>We hope to see you again soon! If you have questions or just want to take a look behind the scenes? </p>
        <p>Do not hesitate to contact us </p>

        <div className="content-img" style={{marginTop: "60px", width: "100%" ,display: "flex", justifyContent: "center"}} >
          <img src='/Img/AboutUsimg.png' alt='' />
        </div>
      </div>

    </div>
  )
}
